import { IS_BUILD } from '../constants/GameConfig';
...
export class PlayGamaSDK {
    ...
    public async showInterstitialAdWithSound(minSeconds: number = 100): Promise<boolean> {
        ...
        try {
            const { AudioSystem } = await import('./Audio');
            // AudioSystem would need scene context which we don't have here
            // For now, just comment out the sound control
        } catch (e) {
        }
